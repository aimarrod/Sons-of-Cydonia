...
public class FlameWallProcessor implements AttackProcessor{
	public AnimatedRenderer renderer;
	public float timer;
	public float lastHit;
	public FlameWallProcessor() {
  		this.renderer = GraphicsLoader.loadFlameWall();
  		this.timer=3600;
  		this.lastHit=3600;
	}
	...
	public boolean collision(Entity attack, Entity victim) {
		...
		return ((lastHit-timer)>0.5f && attackpos.x < victimpos.x + victimbounds.width && attackpos.x + attackbounds.width > victimpos.x && attackpos.y < victimpos.y + victimbounds.height && attackpos.y + attackbounds.height > victimpos.y);
